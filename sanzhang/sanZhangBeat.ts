...
export default class SanZhuangBeat {
    consts = {
    ...
    victory: any;
    failure: any;
    leopardPlayers: any; //豹子
    flowerPlayers: any; //金花
    shunzaPlayers: any; //顺子
    shunKingPlayers: any; //顺金
    earthwormPlayers: any; //地龙
    dracoPlayers: any; //天龙
    erSanWuPlayers: any; // 235
    otherPlayers: any; //其他 单牌
    pairsPlayers: any; //对子  
    lzPlayer1: boolean = false;
    lzPlayer2: boolean = false;
    constructor(self) {
        this.self = self;
        this.victory = {}; //胜利对象
        this.failure = {}; //失败对象
    ...
    beat(gamePlayer1: any, gamePlayer2: any) {
        ...
        if (this.self.roomConfig["lanzi"] != undefined && this.self.roomConfig["lanzi"][0] == 1) { //
            let obj_1: any = {};
            let obj_2: any = {};
            if (gamePlayer1.showCards == true) { //看牌了
                if (gamePlayer1["LzCards"].length > 0) obj_1["LzCards"] = gamePlayer1["LzCards"];
            } else {
                obj_1 = this.LzShowCards(gamePlayer1, true);
            }
            if (gamePlayer2.showCards == true) { //看牌了
                if (gamePlayer2["LzCards"].length > 0) obj_2["LzCards"] = gamePlayer2["LzCards"];
            } else {
                obj_2 = this.LzShowCards(gamePlayer2, true);
            }
            console.log("==33333=beat==obj_1===", obj_1, "==eeeeee=beat==obj_2===", obj_2);
            this._cards_1 = obj_1 == undefined || obj_1["LzCards"] == undefined ? PokerAlgo.createCardsFromIds(gamePlayer1.cards) : PokerAlgo.createCardsFromIds(obj_1["LzCards"]);
            this._cards_2 = obj_2 == undefined || obj_2["LzCards"] == undefined ? PokerAlgo.createCardsFromIds(gamePlayer2.cards) : PokerAlgo.createCardsFromIds(obj_2["LzCards"]);
            ...
            if (obj_1 != undefined && obj_1["LzCards"] != undefined) tmpGamePlayer1.LzCards = obj_1["LzCards"];
            if (obj_2 != undefined && obj_2["LzCards"] != undefined) tmpGamePlayer2.LzCards = obj_2["LzCards"];
    ...
    qunbiBeat() {
        for (let i = 0; i < this.self.beatPlayers.length; i++) {
            ...
            let cards: any = "";
            if (this.self.roomConfig["lanzi"] != undefined && this.self.roomConfig["lanzi"][0] == 1) {
                let showCardsObj: any = {};
                if (player.showCards == true) {
                    if (player.LzCards.length > 0) showCardsObj["LzCards"] = player.LzCards;
                } else {
                    showCardsObj = this.LzShowCards(player);
                }
                if (showCardsObj["LzCards"] != undefined) { //说明有懒子牌
                    //player.cards = showCardsObj["cards"];
                    player.LzCards = showCardsObj["LzCards"];
                    cards = PokerAlgo.createCardsFromIds(player.LzCards);
                } else {
                    cards = PokerAlgo.createCardsFromIds(player.cards);
                }
            } else {
                cards = PokerAlgo.createCardsFromIds(player.cards);
            }
            ...
            if (cardsInfo.info == this.consts["baozi"].info) this.leopardPlayers.push(player); //豹子     
            if (cardsInfo.info == this.consts["shunjin"].info) this.shunKingPlayers.push(player); //顺金            
            if (cardsInfo.info == this.consts["tonghua"].info) this.flowerPlayers.push(player); //金华
            if (cardsInfo.info == this.consts["tianlong"].info) this.dracoPlayers.push(player); //天龙   
            if (cardsInfo.info == this.consts["shunzi"].info) this.shunzaPlayers.push(player);  // 顺子
            if (cardsInfo.info == this.consts["dilong"].info) this.earthwormPlayers.push(player); //地龙    
            if (cardsInfo.info == this.consts["duizi"].info) this.pairsPlayers.push(player); //对子            
            if (cardsInfo.info == this.consts["ersanwu"].info) this.erSanWuPlayers.push(player); //235
            if (cardsInfo.info == this.consts["dantao"].info) this.otherPlayers.push(player); //单牌
        }
        ...
        if (this.shunzaPlayers.length > 0 || this.dracoPlayers.length > 0
            ...
            if ((this.self.roomConfig["shunzi"] != undefined && this.self.roomConfig["shunzi"][0] == 1) || this.flowerPlayers.length == 0) { //顺子大于金华
    ...
    _qunbiBeat(players: Array<any>) {
        console.log("_qunbiBeat ====players.length==", players.length);
        let _player1: any = "";
        let _player2: any = "";
        let victoryPlayers: any = "";
        let cards_1: any = "";
        let cards_2: any = "";
        if (players.length == 1) {  //只有一个玩家胜利
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            if (victoryPlayers.uid == _player1.uid) continue;
            ...
            this._cards_1 = PokerAlgo.createCardsFromIds(cards_1);
            this._cards_2 = PokerAlgo.createCardsFromIds(cards_2);
            this.gamePlayer1 = victoryPlayers;
            this.gamePlayer2 = _player1;
            this._beat();
            if (this.victory.uid == _player1.uid) victoryPlayers = _player1;
        }
    ...
    _getQunBiBeatResult(player) {
        ...
        let results: any = [];
        for (let i = 0; i < this.self.beatPlayers.length; i++) {
            let _player: any = this.self.beatPlayers[i];
            let _cards = ["-1", "-1", "-1"];
            let _obj = { sid: _player.index, uid: _player.uid, cards: _cards }
            if (player.uid == _player.uid) {
                if (player.showCards == true) _obj["cards"] = player.cards;
                results.push(_obj);
                continue;
            }
            // _player.giveup = true;
    ...
    getCheckCards(cards: any, gamePlayer: any) {
        //获取是否是豹子
        let tmpCards = gamePlayer.cards;
        let obj: any = this.getCardsSize(tmpCards);
        let arr = obj.arr;
        if (SanZhangAlgo.isSanTiao(cards)) return this.consts.baozi;
        //获取是否是顺金
        if (SanZhangAlgo.isTongHuaShun(cards)) return this.consts.shunjin;
        //获取是否是同花
        arr.sort(this.compareFunction);
        if (SanZhangAlgo.isTongHua(cards)) {
            if (arr[0] == "14" && arr[1] == "3" && arr[2] == "2") return this.consts.shunjin;
            return this.consts.tonghua
        };
        //获取是否是顺子
        if (arr[0] == "14" && arr[1] == "3" && arr[2] == "2") return this.consts.dilong;
        if (arr[0] == "5" && arr[1] == "3" && arr[2] == "2") return this.consts.ersanwu;
        if (SanZhangAlgo.isDanShunZi(cards)) {
            if (arr[0] == "14" && arr[1] == "13" && arr[2] == "12") return this.consts.tianlong;
            return this.consts.shunzi;
        }
        //获取是否是对子
        if (SanZhangAlgo.isDuiZi(cards)) return this.consts.duizi;
        //单挑
    ...
    _beat() {
        ...
        if (cardsInfo.info == competeInfo.info) { //类型相等 
            ...
            let objCards_1: any = this.getCardsSize(_tmpCards1_);
            let objCards_2: any = this.getCardsSize(_tmpCards2_);
            let tmpCards_1: any = objCards_1.arr;
            let tmpCards_2: any = objCards_2.arr;
            tmpCards_1.sort(this.compareFunction);
            tmpCards_2.sort(this.compareFunction);
            let group_1: any = _.groupBy(tmpCards_1);
            let group_2: any = _.groupBy(tmpCards_2);
            ...
            let shuang1: any = undefined, shuang2: any = undefined, dan1: any = undefined, dan2: any = undefined;
            for (let k in group_1) {
                if (group_1[k].length == 1) dan1 = group_1[k][0];
                if (group_1[k].length == 2) shuang1 = group_1[k][0];
            }
            for (let k in group_2) {
                if (group_2[k].length == 1) dan2 = group_2[k][0];
                if (group_2[k].length == 2) shuang2 = group_2[k][0];
            }
            console.log("=============group_1==",dan1);
            console.log("=============group_2==dan2",dan2)
            if (cardsInfo.info == this.consts.shunjin.info && competeInfo.info == this.consts.shunjin.info) { //顺金对比
                // if(tmpCards_1[0] == "14" && ){}
                let tianlong_1: boolean = tmpCards_1[0] == "14" && tmpCards_1[1] == "13" && tmpCards_1[2] == "12" ? true : false;
                let tianlong_2: boolean = tmpCards_2[0] == "14" && tmpCards_2[1] == "13" && tmpCards_2[2] == "12" ? true : false;
                let dilong_1: boolean = tmpCards_1[0] == "14" && tmpCards_1[1] == "2" && tmpCards_1[2] == "3" ? true : false;
                let dilong_2: boolean = tmpCards_2[0] == "14" && tmpCards_2[1] == "2" && tmpCards_2[2] == "3" ? true : false;
            ...
            if (tmpCards_1.toString() == tmpCards_2.toString()) { //平了
                ...
                if (this.self.roomConfig["bipai2"] != undefined && this.self.roomConfig["bipai2"][0] == 1 && this.self.qiongbi == false) {  //开牌者输
                    console.log("==================tongdakaipaizheshu======");
                    let obj = this.getPlayer2Victory(cardsInfo, competeInfo);
                    this.victory = obj.victory, this.failure = obj.failure;
                    return true;
                }
                if ((this.self.roomConfig["bipai2"] != undefined && this.self.roomConfig["bipai2"][0] == 2) || this.self.qiongbi == true) { //比花色
            ...
            if (cardsInfo.info == this.consts.duizi.info) {  //对子对比
            ...
            for (let i = 0; i < tmpCards_1.length; i++) {
                ...
                if (tmpCards_1[i] == tmpCards_2[i]) continue;
    ...
    getDilong(cardsInfo: any, competeInfo: any) {
    ...
    getTianLong(cardsInfo: any, competeInfo: any, tianlong = false) {
        console.log(
            "=======this.self.roomConfig['tianlongda']", this.self.roomConfig["tianlongda"],
            "=======this.self.roomConfig['tianlong']", this.self.roomConfig["tianlong"],
            "=======this.self.roomConfig['dilong']", this.self.roomConfig["dilong"],
            "=======this.self.roomConfig['dilongda']", this.self.roomConfig["dilongda"]
        );
        if ((this.self.roomConfig["tianlongda"] != undefined && this.self.roomConfig["tianlongda"][0] == 1)
            || (this.self.roomConfig["dilong"] != undefined && this.self.roomConfig["dilong"][0] == 1)) { //地龙最小
        ...
        if ((this.self.roomConfig["tianlongda"] != undefined && this.self.roomConfig["tianlongda"][0] == 1)
            || (this.self.roomConfig["dilongda"] != undefined && this.self.roomConfig["dilongda"][0] == 1)) {  //地龙大于顺子
    ...
    checkFond(gamePlayer: any) {
        ...
        if (gamePlayer.menpai > 0) {
            ...
            let cards = PokerAlgo.createCardsFromIds(_cards);
            if (SanZhangAlgo.isSanTiao(cards)) {
                if (gamePlayer.LzCards.length == 0) return this.self.gameConfig["multiple"]["baozi"];
            ...
            if (SanZhangAlgo.isTongHuaShun(cards) && gamePlayer.LzCards.length == 0) return this.self.gameConfig["multiple"]["sunjin"];
            if (this.self.isErSanWuEatBaoZi == true) return this.self.gameConfig["multiple"]["ersanwu"];
        }
        return 0;
    }
    checkLzCards(gamePlayer: any) {
        // let flag = false;
        if (gamePlayer.cards) {
            for (let i = 0; i < gamePlayer.cards.length; i++) {
    ...
    //获取懒子手牌
    LzShowCards(gamePlayer: any, isMenPai = false) {
        let cards = gamePlayer.cards.slice();
        let arr: Array<string> = []; //懒子牌
        let brandValue: Array<any> = [];
    ...
    getLzCard(cards: Array<string>, gamePlayer: any, brandValue: Array<string>, lzCard: any) {
        ...
        if (cardsInfo.info == this.consts.tonghua.info) isTongHua = true;
        // console.log("cardsInfo.info=====",cardsInfo.info);

        if (cardsInfo.info == this.consts.duizi.info || cardsInfo.info == this.consts.baozi.info) { //豹子的情况
        ...
        if (cardsInfo.info == this.consts.shunzi.info || cardsInfo.info == this.consts.shunjin.info
            || cardsInfo.info == this.consts.tianlong.info || cardsInfo.info == this.consts.dilong.info) { //顺子的情况
            // let lzCards: any = [];
            console.log("===cardsInfo.info==", cardsInfo.info);
            ...
            if (brandValue[0] == "3" && brandValue[1] == "2") {
                if ((this.self.roomConfig["dilong"] != undefined && this.self.roomConfig["dilong"][0] == 1) ||
                    (this.self.roomConfig["tianlong"] != undefined && this.self.roomConfig["tianlong"][0] == 1)) {  //地龙最小
            ...
            if (num.toString() == "14") type = type != this.consts.shunjin.info ? this.consts.tianlong.info : this.consts.shunjin.info;
            return { cards: cards, LzCards: LzCards, type: type };
        }
    ...
    sortCardsPlayer(cards: Array<string>) {
        let obj: any = {};
        let LzCard: any = [];
        let duizi: any = false;
        let baozi: any = false;
        ...
        if (duizi == false && baozi == false) _cards = this._getSortDanCards(keyArr, obj);

        return _cards.concat(LzCard);
    }
    _getSortDanCards(keyArr: any, obj: any) {
        let _cards: any = [];
    ...
    _getSortDuiBaoCards(arr: any, duizi: any) {
        let _cards: any = [];
